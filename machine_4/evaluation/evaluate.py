...
def evaluate_file(cfg, raw_path: Path, tasks_by_id: Dict[str, Any],
    ...
    with raw_path.open("r", encoding="utf-8") as fh:
        for line in fh:
            ...
            # Primary quality, chosen by the task's declared evaluation_type.
            #
            # BUG FIX: the previous version used exact_match whenever an
            # expected_answer existed. For rubric tasks the expected_answer
            # is a concatenation of the gold facts, which never appears
            # verbatim in a generated answer, so exact_match was always 0
            # and -- being 0 rather than None -- it won the priority chain
            # and forced quality to 0 for every record, discarding the
            # gold-fact and judge signal that had been computed correctly.
            eval_type = getattr(task, "evaluation_type", "rubric")
            if eval_type == "exact_match" and em is not None:
                quality = float(em)
            elif gfc is not None:
                quality = float(gfc)      # deterministic, preferred
            elif rubric is not None:
                quality = float(rubric)   # judge, secondary
            elif em is not None:
                quality = float(em)
            else:
                quality = None
            ...
            evaluated = {
                ...
                "quality_source": ("exact_match" if (getattr(task, "evaluation_type", "rubric") == "exact_match" and em is not None)
                                   else "gold_fact_coverage" if gfc is not None
                                   else "judge_rubric" if rubric is not None else None),
